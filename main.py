...
class FedExClient:
    ...
    def track_multiple(self, tracking_numbers: list):
              if not self.access_token:
                        ...
                        tracking_info = [{"trackingNumberInfo": {"trackingNumber": tn}} for tn in tracking_numbers[:30]]
        response = requests.post(
                      url,
                      headers={
                      ...
                      json={"includeDetailedScans": True, "trackingInfo": tracking_info},
                      timeout=60
        )
        if response.status_code == 200:
                  ...
                  for track_result in result.get("output", {}).get("completeTrackResults", []):
                                ...
                                for detail in track_result.get("trackResults", []):
                                                  ...
                                                  for event in detail.get("scanEvents", []):
                                                                    ...
                                                                    parsed.append({
                                                      "tracking": tracking_number, "status": status, "label_date": label_date,
                                                      "ship_date": ship_date, "delivery_date": delivery_date,
                                                      "origin_city": origin.get("city", ""), "origin_state": origin.get("stateOrProvinceCode", ""),
...
def generate_excel(data: list, client_map: dict) -> str:
      wb = Workbook()
    ws = wb.active
    ...
    headers = ["Cliente", "Tracking", "Status", "Label Date", "Ship Date", "Days After Ship", "Working Days", "Days After Label", "Origin City", "Origin State", "Origin Country", "SonIA"]
    ...
    for row_idx, item in enumerate(data, 2):
        ...
        if "Delivered" in status and deliv_dt and ship_dt:
            ...
            label_text = "0"
            sonia = "Verificar"
        row_data = [client_map.get(tn, ""), tn, status, label_dt.strftime('%Y-%m-%d') if label_dt else '', ship_dt.strftime('%Y-%m-%d') if ship_dt else '', days_ship, work_days, label_text, item.get('origin_city', ''), item.get('origin_state', ''), item.get('origin_country', ''), sonia]
...
async def track_shipments(file: UploadFile = File(...)):
    ...
    try:
        ...
        return FileResponse(output_path, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", filename="FedEx_Tracking_Report_SonIA.xlsx")
finally:
